...
class Selection:
    ...
    def binary_tournament(self,population, fitness_list, num_samples):
        samples = list()
        sample_indices = self.seed.choice(np.arange(len(fitness_list)), 2*num_samples)
        sample_pop = population[sample_indices,:]
        # print(f"sample len =  {len(sample_pop)}")
       
        for i in range(0, len(sample_pop),2):
            samples.append(sample_pop[i,:]) if fitness_list[sample_indices[i]] > fitness_list[sample_indices[i+1]] else samples.append(sample_pop[i+1,:])
