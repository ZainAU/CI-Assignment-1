...
class TSP_EA(EA):
    def __init__(self, seed=rng, population_size=30, dataset="qa194.tsp",
        ...
        super().__init__(seed, population_size, dataset, mutation_rate, offspring_number,  num_generations,
                         Iterations, parent_selection_method, survival_selection, mutation_type, optimization_type)
        population = self.population_init()
        return

    def population_init(self):
        self.dataset = self.dataLoader()
        # Not adding one, This will make indexing a little easier
        self.population = np.tile(np.arange(self.chromosome_length), [
    ...
    def dataLoader(self):
        ...
        for i in range(self.chromosome_length):
            for j in range(self.chromosome_length):
                distance[i, j] = distance[j, i] = self.ManhattanDistance(
                    lst[i][1:], lst[j][1:])
...
>>>>>>> f3c133e989c3d2ff3070590407b83ebba482fcef
            ...
            average_best_fitness = np.zeros(
                [self.Iterations, self.num_generations])
            ...
            fig.savefig(
                f'TSP_fig/Best-fit-{int(Best)}-parent-{self.parent_selection_method}-survival-{self.survival_Selection_method}-{self.population_size}-{self.mutation_rate}-iteration-{i}.png')
            # plt.show()
...
if __name__ == '__main__':
    ...
    selection_criteria = [('FPS', 'Random'), ('BT', 'Trunc'),
                          ('Trunc', 'Trunc'), ('Random', 'Random'), ('FPS', 'RBS')]
